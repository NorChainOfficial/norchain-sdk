...
describe('WalletService', () => {
  ...
  beforeEach(async () => {
    ...
    try {
      ...
      const providers = (module as any).container?.modules?.values() || [];
      console.error('Available providers:', Array.from(providers).map((m: any) => m?.providers?.keys() || []));
      console.error('Error getting WalletService:', error);
      throw error;
    }
  });
